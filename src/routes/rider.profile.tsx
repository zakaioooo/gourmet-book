...
function RiderProfile() {
  ...
  const submit = () => {
    ...
    const id = saveRider({ ...form, name: form.name.trim(), phone: form.phone.trim() });
    setCurrentRider(id);
    setMode("edit");
    toast.success(
  ...
  const shareLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setRiderLocation(rider.id, { lat: pos.coords.latitude, lng: pos.coords.longitude }, true);
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            {rider ? (
              <>
                ...
                <GhostButton onClick={shareLocation}>
                  <MapPin className="h-3.5 w-3.5" /> Share live location
                </GhostButton>
                <GhostButton onClick={() => setRiderLocation(rider.id, null, false)}>
                  Stop sharing
                </GhostButton>
              </>
            ) : null}
