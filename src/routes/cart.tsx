...
function CartPage() {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const address = { id: crypto.randomUUID(), ...form, ...coords } as Address & {
        lat: number;
        lng: number;
      };
      const first = selected[0]!;
      const code = `MG-${Math.floor(100000 + Math.random() * 899999)}`;
      ...
      // Keep the profile card in sync so users never re-type their details.
      await saveProfile({
        id: user.id,
        full_name: form.name,
        phone: form.phone,
        city: form.city,
        street: [form.street, form.area].filter(Boolean).join(", "),
        avatar_url: null,
      }).catch(() => undefined);

      clearSelected();
      toast.success(`Order ${code} placed`, { description: "Live tracking shuru ho gayi." });
