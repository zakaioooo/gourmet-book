    ...
    <div className="space-y-6">
      ...
      <Panel title="Journey" subtitle="Tap a stage to move the order there">
        <div className="grid gap-2 sm:grid-cols-6">
          {STATUS_FLOW.map((s, i) => {
            const done = stageIndex >= i && order.status !== "cancelled";
            return (
              <button
                key={s}
                onClick={() => {
                ...
                className={
                    ...
                    ? "rounded-2xl border border-lux/50 bg-gradient-to-b from-lux/25 to-transparent px-3 py-3 text-[11px] font-black uppercase tracking-[0.12em] text-lux"
                    : "rounded-2xl border border-lux/15 px-3 py-3 text-[11px] font-black uppercase tracking-[0.12em] text-slate-dim transition hover:border-lux/40 hover:text-frost"
                }
              >
                {STATUS_LABEL[s]}
              </button>
          ...
          <div className="rounded-2xl border border-red-400/25 px-3 py-3">
            ...
            <button
              onClick={() => {
                setOrderStatus(order.id, "cancelled", cancelReason || "No reason given");
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <GhostButton
                    onClick={() => {
                      verifyPayment(order.id, { status: "pending" });
                  ...
                  <GhostButton
                    onClick={() => {
                      verifyPayment(order.id, { status: "refunded", amountPaid: 0 });
                  ...
                  <DangerButton
                    onClick={() => {
                      verifyPayment(order.id, { status: "failed", amountPaid: 0 });
          ...
          <Panel title="Delivery assignment">
            <Field label="Assigned rider">
              <select
                value={order.riderId ?? ""}
                onChange={(e) => {
                  assignRider(order.id, e.target.value || null);
                  toast.success(e.target.value ? "Rider assigned" : "Rider removed");
                }}
                className={fieldClass}
              >
            ...
            <div className="space-y-3">
              <Field label="Priority">
                <select
                  ...
                  onChange={(e) => {
                    setPriority(order.id, e.target.value as Priority);
